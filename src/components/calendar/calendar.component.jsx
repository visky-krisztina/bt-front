...
const Calendar = ({ preloadedEvents }) => {
    ...
    const Navigation = ({ date, setDate }) => {
        ...
        <div className="calendar-navigation">
            ...
            <div className="monthAndYear">
                <h2>{MONTHS[date.getMonth()].toUpperCase()} / {date.getFullYear()}</h2>
            </div>
    ...
    const DayLabels = () => {
        ...
        return <div className="dayLabel" key={index}>{dayLabel}</div>
        })
    }
    ...
    const MiniEvent = ({ event }) => {
        ...
        return (
          <div 
            className={`miniEvent ${event.type ? event.type.toLowerCase() : "alkalom"}`} >
